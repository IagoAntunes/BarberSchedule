...
class ScheduledTimeDataSource implements IScheduledTimeDataSource {
  ...
  @override
  Future<ResponseDataObject> scheduleTime(ScheduleTimeRequest request) async {
    try {
      final result =
          await _httpService.post(AppApiRoutes.createOrder(), request.toMap());
      if (result!.statusCode == HttpStatus.created ||
          result.statusCode == HttpStatus.noContent) {
        return ResponseData.success(result.data);
      }
      return ResponseData.error({'data': 'error'});
    } catch (e) {
